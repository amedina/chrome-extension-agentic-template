...
- `popup/**` - Quick-access popup panel (HTML, CSS, JS).
- `scripts/**` - Service worker and content script.
- `styles/**` - Shared CSS variables and base styles.
- `.claude/skills/**` - Claude Code skills for each AI API.
...
<list commands you'll use to validate with 100% confidence the chore is complete with zero regressions. every command must execute without errors so be specific about what you want to run to validate the chore is complete with zero regressions. Since this is a no-build-step Chrome extension, focus on static checks and manual verification steps.>
- Verify `manifest.json` is valid JSON: `python3 -c "import json; json.load(open('manifest.json'))"`
- Verify no syntax errors in JS files: `node --check sidepanel/sidepanel.js scripts/service-worker.js scripts/content.js popup/popup.js`
- Verify HTML files are well-formed and reference correct assets
